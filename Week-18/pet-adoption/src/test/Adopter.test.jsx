...
describe("AdopterData Component", () => {
  ...
  it("renders table with correct headers", () => {
    const { container } = render(
      <AdopterData formData={mockFormData} handleGoBack={mockHandleGoBack} />,
    );
  ...
  it("displays form data correctly in table rows", () => {
    ...
    expect(cells[2]).toHaveTextContent(mockFormData[0].yourName);
  ...
  it("renders empty table when no data is provided", () => {
    const { container } = render(
      <AdopterData formData={[]} handleGoBack={mockHandleGoBack} />,
    );
